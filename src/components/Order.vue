      ...
      <el-table :data="userlist" border stripe>
        ...
        <el-table-column align="center" label="类别">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.num == 1">素菜</span>
        ...
        <el-table-column label="操作" width="150px">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              icon="el-icon-delete"
              size="mini"
              @click="removeUserById(scope.row.dingdan_id)"
            ></el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    async getUserList() {
      const { data: res } = await this.$http.get("getDingdan");
    ...
    async removeUserById(dingdan_id) {
      ...
      const { data: res } = await this.$http.get("delOrder", {
        params: {
          dingdan_id,
        },
      });
    ...
    // 确认修改
    async editUserInfo() {
      const { data: res } = await this.$http.get("updateOrder", {
        params: {
